<template>
  <edit-modal base-modal-custom-class="overflow-hidden min-h-screen h-full overflow-y-auto"
    :title="`${professionalId ? 'Edit' : 'Add'} A Professional`" @close="$emit('handle-add-professional', false)"
    :show-modal="showAddProfessionalModal">
    <form class="flex flex-col gap-5 items-center w-full text-center" slot="form"
      @submit.prevent="handleAddProfessionalSubmit">

      <input
        class="rounded focus-visible:outline-none text-blue__cl text-sm font-normal p-4 w-full bg-gray__bg max-w-[400px]"
        v-model="name" placeholder="Professional's Name" type="text" required />
      ...
      <div class="w-full max-w-[400px]">
        <multiselect v-model="selectedServices" :options="allServices" :multiple="true" :clear-on-select="false"
          :preserve-search="true" :placeholder="`Select The Services ${name ? name : 'This Professional'} Offers`"
          label="name" track-by="name" />
      </div>
      <select
        class="rounded focus-visible:outline-none text-blue__cl text-sm font-normal p-4 w-full bg-gray__bg max-w-[400px]"
        v-model="selectedWeekDay" @change="onWeekDaysChange">
        <option value="" disabled selected>Working Days</option>
        <option v-for="option in computedWeekDays" :value="option" :key="option" :selected="option === selectedWeekDay">{{
          option }}</option>
      </select>
        ...
        <div v-for="(schedule, index) in workingSchedule" :class="index < workingSchedule.length - 1 ? 'mb-5' : ''">
          ...
          <div class="flex flex-col gap-1">
            <div class="flex flex-col gap-1">
              ...
              <date-picker type="time" :time-picker-options="{
              ...
              }" range v-model="schedule['startEndTime']" valueType="format" placeholder="hh:mm a"
                format="hh:mm a"></date-picker>
            </div>
            <select
              class="rounded focus-visible:outline-none text-blue__cl text-sm font-normal p-4 w-full bg-gray__bg max-w-[400px]"
              v-model="schedule['type']" required>
              <option value="" disabled selected>Select Schedule Type</option>
              <option v-for="option in professionalScheduleTypeArray" :value="option" :key="option"
                :selected="option === schedule['type']">{{ option }}</option>
            </select>
      ...
      <base-button is-submit-type custom-classes="w-full max-w-[400px] mt-5"
        v-if="!(isHandleAddProfessionalLoading || isHandleEditProfessionalLoading)">
