<template>
  <section class="mx-10 mb-5">
      ...
      <div class="bg-peach__bg w-full py-8 px-10 rounded-3xl">
        ...
        <div class="bg-white my-6 py-8 rounded-[24px]">
          ...
          <div class="flex items-center justify-between border-b border-gray__cl mb-7 pb-7 px-8">
            ...
            <button @click="() => handleEditWorkingSchedule(true)"
              class="font-normal text-blue__cl text-xl hover:opacity-90">
              Manage </button>
          </div>
          <div class="flex items-center justify-between border-b border-gray__cl mb-7 pb-7 px-8">
            <div class="flex items-center gap-10">
            ...
            <div class="flex items-center gap-2.5">
              <button @click="() => handleManagePromoCodes(true)"
                class="font-normal text-blue__cl text-xl hover:opacity-90">View</button>
              <span class="text-blue__cl text-xl">|</span>
              <button @click="() => handleAddPromoCodesModal(true)"
                class="font-normal text-blue__cl text-xl hover:opacity-90">Add</button>
            </div>
          </div>
        </div>
      </div>
    ...
    <edit-working-schedule-modal :show-working-schedule-modal="showEditWorkingScheduleModal"
      @handle-edit-working-schedule="handleEditWorkingSchedule"
      :business-working-schedule="businessSchedule.schedule || []" :schedule-id="businessSchedule.id || ''"
      :key="businessSchedule.id" @confirm-business-schedule="confirmBusinessSchedule" />

    <!--   Manage Promo Codes Modal-->
    <manage-promo-codes :show-manage-promo-codes-modal="showManagePromoCodesModal"
      @handle-manage-promo-codes="handleManagePromoCodes" :promo-codes="promoCodes || []"
      @confirm-promo-code-deleted="confirmBusinessSchedule" />

    <!--   Add Promo Codes Modal-->
    <add-promo-code :show-add-promo-codes-modal="showAddPromoCodesModal"
      @handle-add-promo-codes="handleAddPromoCodesModal" @confirm-promo-code-added="confirmBusinessSchedule" />
  </section>
</template>
