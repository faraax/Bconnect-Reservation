  ...
  <div>
    ...
    <client-only>
      ...
      <div class="px-48 relative mt-10">
        <h1 class="text-blue__cl text-4xl font-bold absolute top-0 w-[24%]">Appointment Booking {{ ((isUserLoggedIn ===
          'Customer') && activeTabIndex === 2) ? 'Completed Successfully' : '' }}</h1>
        <form-wizard ref="wizard" :start-index.sync="activeTabIndex" shape="square" color="#20a0ff" error-color="#ff4949"
          title="" subtitle="" :hide-buttons="activeTabIndex === 2">
          <tab-content :beforeChange="beforeServiceTabChange" title="Services" icon="">
            ...
            <div class="pt-5 pb-12 grid grid-cols-3 gap-10"
              v-if="(Array.isArray(businessServices.data) ? businessServices.data.length : 0) > 0">
          ...
          <tab-content title="Book Professional" icon="" :beforeChange="beforeBookProfessionalTabChange">
            <div v-if="businessProfessionalsByService && businessProfessionalsByService.length <= 0" class="my-5">
              <div class="p-14 bg-peach__bg shadow border-0 rounded-[20px]">
                <h1 class="text-center text-dark__blue__cl font-bold text-4xl">No Professionals Available For
                  {{ currentServiceForProfessional.name }}.</h1>
              </div>
            </div>
            <div v-if="businessProfessionalsByService && businessProfessionalsByService.length > 0">
              <h6 class="pt-5 pb-5 text-2xl font-normal text-dark__blue__cl">Booking for <span>{{
                currentServiceForProfessional.name || '' }}</span></h6>
              <scheduling-calendar ref="schedulingCalendar" :is-editable="!isLockBookingEditing"
                :business-schedule="businessSchedule && businessSchedule.schedule"
                :professionals-by-service="businessProfessionalsByService" :existing-events="businessBookingsByService"
                @handle-event-push="handleNewEventPush" :current-service="currentServiceForProfessional" />
            </div>

            <error-message :is-error="isBookingEmpty"
              :message="`Please Book A Slot For ${currentServiceForProfessional.name || ''} To Continue`" />
          </tab-content>
          <tab-content title="Confirm Booking" icon="">
            <div class="pb-12 pt-10  flex gap-16" v-if="(isUserLoggedIn !== 'Customer') && !isBookingCompleted">
            ...
            <div class="pb-12 pt-10  flex gap-16" v-if="(isUserLoggedIn === 'Customer') && isBookingCompleted">
              <div class="flex flex-col pt-10 border-t-2 border-dark__blue__cl">
                <h2 class="text-dark__blue__cl font-bold text-4xl pb-10"><span class="text-blue__cl">{{ addEvent &&
                  addEvent.length > 1 ? `${addEvent[0] && addEvent[0].professionalName} and others` : addEvent[0] &&
                addEvent[0].professionalName }}</span> will be waiting for you on
                  <span class="text-blue__cl">{{ Date.parse(new Date(addEvent[0] && addEvent[0].start).toDateString()) ?
                    new Date(addEvent[0] && addEvent[0].start).toDateString() : '' }}</span>
                </h2>
                <h3 class="text-dark__blue__cl text-xl pb-10">You will receive a confirmation SMS shortly.</h3>
                <base-button custom-classes="mb-5"
                  :click-handler="() => $router.push(`/reservation/${$route.params.business}${routes.BOOKING_HISTORY}`)">View
                  Booking History</base-button>
        ...
        <!--    Login Modal-->
        <login-modal :show-login-modal="showLoginModal" @handle-login="handleLogin" is-booking
          @handle-confirm-booking="handleConfirmBooking" />
        <!--    Signup Modal-->
        <signup-modal :show-signup-modal="showSignupModal" @handle-signup="handleSignup" is-booking
          @handle-confirm-booking="handleConfirmBooking" />
        <!--    Confirm Booking Modal-->
        <confirm-booking-modal :final-booking-data="addEvent" :user-id="loggedInUserId"
          :show-confirm-booking-modal="showConfirmBookingModal" @handle-confirm-booking="handleConfirmBooking"
